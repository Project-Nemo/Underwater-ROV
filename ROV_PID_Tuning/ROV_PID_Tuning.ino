...
// Gyroscope and Accelerometer objects for the station keeping
GYRO gyroscope;
ACCEL accelerometer;
float accPitch = 0; // pitch calculated from the accelerometer
float accRoll = 0; // roll calculated from the accelerometer
...
int leftVal; // Left thruster value
int rightVal; // Right thruster value
// Boundaries for station keeping; Angles between the bounds don't station keep
int low_bound = -10; 
int high_bound = 10;
...
void setup()
{
  ...
  delay(5000); // to settle calibration
...
void loop() {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    read_IMU();
    pid();
    if (isNotControllingROV(rxdata.upLraw, rxdata.upRraw, rxdata.HLraw, rxdata.HRraw, neutral)) {
      stationKeepRoll();
    } else {
...
void stationKeepRoll() {
  // if the angle is not between -10 and 10
...
void read_IMU() {
  ...
  // roll is now any value between 0 to 85 to 0.
  // negative rolling to the left, positive to the right
  if (accPitch < 0) {
    // will give a value between -85 and 0, -85 being the horizontal on the left and 0 being the neutral
